...
#define _R(NAME, MAC, RCR, MASK, JumFrameSz)                                   \
	{                                                                      \
		.name = NAME, .mcfg = MAC, .RCR_Cfg = RCR,                     \
		.RxConfigMask = MASK, .jumbo_frame_sz = JumFrameSz             \
	}
...
static const struct {
	...
	_R("RTL8125A", CFG_METHOD_2,
	   BIT_30 | EnableInnerVlan | EnableOuterVlan |
		   (RX_DMA_BURST << RxCfgDMAShift),
	   0xff7e5880, Jumbo_Frame_9k),

	_R("RTL8125A", CFG_METHOD_3,
	   BIT_30 | EnableInnerVlan | EnableOuterVlan |
		   (RX_DMA_BURST << RxCfgDMAShift),
	   0xff7e5880, Jumbo_Frame_9k),

	_R("RTL8125B", CFG_METHOD_4,
	   BIT_30 | RxCfg_pause_slot_en | EnableInnerVlan | EnableOuterVlan |
		   (RX_DMA_BURST << RxCfgDMAShift),
	   0xff7e5880, Jumbo_Frame_9k),

	_R("RTL8125B", CFG_METHOD_5,
	   BIT_30 | RxCfg_pause_slot_en | EnableInnerVlan | EnableOuterVlan |
		   (RX_DMA_BURST << RxCfgDMAShift),
	   0xff7e5880, Jumbo_Frame_9k),

	_R("RTL8168KB", CFG_METHOD_6,
	   BIT_30 | EnableInnerVlan | EnableOuterVlan |
		   (RX_DMA_BURST << RxCfgDMAShift),
	   0xff7e5880, Jumbo_Frame_9k),

	_R("RTL8168KB", CFG_METHOD_7,
	   BIT_30 | RxCfg_pause_slot_en | EnableInnerVlan | EnableOuterVlan |
		   (RX_DMA_BURST << RxCfgDMAShift),
	   0xff7e5880, Jumbo_Frame_9k),
...
static int rtl8125_rx_interrupt(struct net_device *, struct rtl8125_private *,
				struct rtl8125_rx_ring *, napi_budget);
static int rtl8125_tx_interrupt(struct rtl8125_tx_ring *ring, int budget);
static int rtl8125_tx_interrupt_with_vector(struct rtl8125_private *tp,
					    const int message_id, int budget);
...
static int rtl8125_set_speed(struct net_device *dev, u8 autoneg, u32 speed,
			     u8 duplex, u32 adv);
...
#endif
		"dev_addr\t%pM\n",
		MODULENAME, RTL8125_VERSION, tp->mcfg, tp->chipset,
		rtl_chip_info[tp->chipset].name, dev->mtu,
		tp->rx_ring[0].num_rx_desc, tp->rx_ring[0].cur_rx,
		tp->rx_ring[0].dirty_rx, tp->rx_ring[1].cur_rx,
		tp->rx_ring[1].dirty_rx, tp->rx_ring[2].cur_rx,
		tp->rx_ring[2].dirty_rx, tp->rx_ring[3].cur_rx,
		tp->rx_ring[3].dirty_rx, tp->tx_ring[0].num_tx_desc,
		tp->tx_ring[0].cur_tx, tp->tx_ring[0].dirty_tx,
		tp->tx_ring[1].cur_tx, tp->tx_ring[1].dirty_tx, tp->rx_buf_sz,
		tp->esd_flag, tp->pci_cfg_is_read, tp->rtl8125_rx_config,
		tp->cp_cmd, tp->intr_mask, tp->timer_intr_mask, tp->wol_enabled,
		tp->wol_opts, tp->efuse_ver, tp->eeprom_type, tp->autoneg,
		tp->duplex, tp->speed, tp->advertising, tp->eeprom_len,
		tp->cur_page, tp->bios_setting, tp->features,
		tp->org_pci_offset_99, tp->org_pci_offset_180,
		tp->issue_offset_99_event, tp->org_pci_offset_80,
		tp->org_pci_offset_81, tp->use_timer_interrrupt,
		tp->HwIcVerUnknown, tp->NotWrRamCodeToMicroP,
		tp->NotWrMcuPatchCode, tp->HwHasWrRamCodeToMicroP,
		tp->sw_ram_code_ver, tp->hw_ram_code_ver, tp->rtk_enable_diag,
		tp->ShortPacketSwChecksum, tp->UseSwPaddingShortPkt,
		tp->RequireRduNonStopPatch, tp->RequireAdcBiasPatch,
		tp->AdcBiasPatchIoffset, tp->RequireAdjustUpsTxLinkPulseTiming,
		tp->SwrCnt1msIni, tp->HwSuppNowIsOobVer, tp->HwFiberModeVer,
		tp->HwFiberStat, tp->HwSwitchMdiToFiber, tp->NicCustLedValue,
		tp->RequiredSecLanDonglePatch, tp->HwSuppDashVer, tp->DASH,
		tp->dash_printer_enabled, tp->HwSuppKCPOffloadVer, speed_mode,
		duplex_mode, autoneg_mode, advertising_mode, aspm, s5wol,
		s5_keep_curr_mac, tp->eee.eee_enabled, hwoptimize,
		proc_init_num, s0_magic_packet, tp->HwSuppMagicPktVer,
		tp->HwSuppLinkChgWakeUpVer, tp->HwSuppD0SpeedUpVer,
		tp->D0SpeedUpSpeed, tp->HwSuppCheckPhyDisableModeVer,
		tp->HwPkgDet, tp->HwSuppTxNoCloseVer, tp->EnableTxNoClose,
		tp->tx_ring[0].NextHwDesCloPtr, tp->tx_ring[0].BeginHwDesCloPtr,
		tp->tx_ring[1].NextHwDesCloPtr, tp->tx_ring[1].BeginHwDesCloPtr,
		tp->InitRxDescType, tp->RxDescLength, tp->num_rx_rings,
		tp->num_tx_rings, tp->tot_rx_rings, tp->tot_tx_rings,
		tp->EnableRss, tp->EnablePtp, tp->ptp_master_mode,
		tp->min_irq_nvecs, tp->irq_nvecs, tp->ring_lib_enabled,
		tp->HwSuppIsrVer, tp->HwCurrIsrVer,
#ifdef ENABLE_PTP_SUPPORT
		tp->tx_hwtstamp_timeouts, tp->tx_hwtstamp_skipped,
#endif
		tp->random_mac, tp->org_mac_addr,
#if LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 13)
		dev->perm_addr,
#endif
		dev->dev_addr);
				  ...
				  int count, int *eof, void *data)
{
	...
	len += snprintf(
		...
		le32_to_cpu(counters->rx_mac_missed),
		le32_to_cpu(counters->rx_tcam_dropped),
		le32_to_cpu(counters->tdu), le32_to_cpu(counters->rdu), );
...
static int proc_get_temperature(char *page, char **start, off_t offset,
				int count, int *eof, void *data)
{
	struct net_device *dev = data;
	struct rtl8125_private *tp = netdev_priv(dev);
	...
	if (ts_digout <= 512) {
		tj = ts_digout / 2;
		len += snprintf(page + len, count - len, "Cel:%d\n", tj);
		fah = tj * (9 / 5) + 32;
		len += snprintf(page + len, count - len, "Fah:%d\n", fah);
...
static void mdio_direct_write_phy_ocp(struct rtl8125_private *tp, u16 RegAddr,
				      u16 value)
{
...
static void mdio_real_write(struct rtl8125_private *tp, u16 RegAddr, u16 value)
{
	if (RegAddr == 0x1F) {
		tp->cur_page = value;
		return;
...
static u32 mdio_real_read(struct rtl8125_private *tp, u16 RegAddr)
{
...
static void ClearAndSetEthPhyBit(struct rtl8125_private *tp, u8 addr,
...
static void ClearAndSetEthPhyOcpBit(struct rtl8125_private *tp, u16 addr,
				    u16 clearmask, u16 setmask)
{
	u16 PhyRegValue;
...
{
	ClearAndSetEthPhyOcpBit(tp, addr, mask, 0);
}

void SetEthPhyOcpBit(struct rtl8125_private *tp, u16 addr, u16 mask)
{
	ClearAndSetEthPhyOcpBit(tp, addr, 0, mask);
}
...
static void ClearAndSetMcuAccessRegBit(struct rtl8125_private *tp, u16 addr,
				       u16 clearmask, u16 setmask)
{
	u16 PhyRegValue;
...
static void ClearMcuAccessRegBit(struct rtl8125_private *tp, u16 addr, u16 mask)
{
	ClearAndSetMcuAccessRegBit(tp, addr, mask, 0);
}

static void SetMcuAccessRegBit(struct rtl8125_private *tp, u16 addr, u16 mask)
{
	ClearAndSetMcuAccessRegBit(tp, addr, 0, mask);
}

u32 rtl8125_ocp_read_with_oob_base_address(struct rtl8125_private *tp, u16 addr,
					   u8 len, const u32 base_address)
{
	return rtl8125_eri_read_with_oob_base_address(tp, addr, len, ERIAR_OOB,
						      base_address);
}
...
u32 rtl8125_ocp_read(struct rtl8125_private *tp, u16 addr, u8 len)
{
	...
	if (HW_DASH_SUPPORT_TYPE_2(tp))
		value = rtl8125_ocp_read_with_oob_base_address(tp, addr, len,
							       NO_BASE_ADDRESS);
	else if (HW_DASH_SUPPORT_TYPE_3(tp))
		value = rtl8125_ocp_read_with_oob_base_address(
			tp, addr, len, RTL8168FP_OOBMAC_BASE);

	return value;
}

u32 rtl8125_ocp_write_with_oob_base_address(struct rtl8125_private *tp,
...
{
	if (HW_DASH_SUPPORT_TYPE_2(tp))
		rtl8125_ocp_write_with_oob_base_address(tp, addr, len, value,
							NO_BASE_ADDRESS);
	else if (HW_DASH_SUPPORT_TYPE_3(tp))
		rtl8125_ocp_write_with_oob_base_address(tp, addr, len, value,
...
u16 rtl8125_ephy_read(struct rtl8125_private *tp, int RegAddr)
{
	...
	RTL_W32(tp, EPHYAR,
		EPHYAR_Read | (RegAddr & EPHYAR_Reg_Mask_v2)
				      << EPHYAR_Reg_shift);
...
static void ClearAndSetPCIePhyBit(struct rtl8125_private *tp, u8 addr,
				  u16 clearmask, u16 setmask)
{
	u16 EphyValue;
...
static u32 rtl8125_csi_other_fun_read(struct rtl8125_private *tp,
...
static void rtl8125_csi_other_fun_write(struct rtl8125_private *tp,
					u8 multi_fun_sel_bit, u32 addr,
					u32 value)
{
	u32 cmd;
...
static u32 rtl8125_csi_read(struct rtl8125_private *tp, u32 addr)
{
...
static void rtl8125_csi_write(struct rtl8125_private *tp, u32 addr, u32 value)
{
	u8 multi_fun_sel_bit;
...
static u8 rtl8125_csi_fun0_read_byte(struct rtl8125_private *tp, u32 addr)
{
	u8 RetVal = 0;
...
static void rtl8125_csi_fun0_write_byte(struct rtl8125_private *tp, u32 addr,
					u8 value)
{
	if (tp->mcfg == CFG_METHOD_DEFAULT) {
		struct pci_dev *pdev = tp->pci_dev;
...
u32 rtl8125_eri_read_with_oob_base_address(struct rtl8125_private *tp, int addr,
					   int len, int type,
					   const u32 base_address)
{
...
int rtl8125_eri_write_with_oob_base_address(struct rtl8125_private *tp,
					    ...
					    int type, const u32 base_address)
{
	...
	while (len > 0) {
		...
		value1 = rtl8125_eri_read_with_oob_base_address(
...
int rtl8125_eri_write(struct rtl8125_private *tp, int addr, int len, u32 value,
		      int type)
{
	return rtl8125_eri_write_with_oob_base_address(tp, addr, len, value,
...
static void rtl8125_enable_rxdvgate(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static void rtl8125_disable_rxdvgate(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static u8 rtl8125_is_gpio_low(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	u8 gpio_low = FALSE;
...
static u8 rtl8125_is_phy_disable_mode_enabled(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static u8 rtl8125_is_in_phy_disable_mode(struct net_device *dev)
{
	u8 in_phy_disable_mode = FALSE;

	if (rtl8125_is_phy_disable_mode_enabled(dev) &&
	    rtl8125_is_gpio_low(dev))
...
static bool rtl8125_stop_all_request(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	int i;
...
static inline void rtl8125_enable_dash2_interrupt(struct rtl8125_private *tp)
{
	if (!tp->DASH)
		return;
...
static inline void rtl8125_disable_dash2_interrupt(struct rtl8125_private *tp)
{
	if (!tp->DASH)
		return;
...
void rtl8125_enable_hw_linkchg_interrupt(struct rtl8125_private *tp)
{
	...
	case 2:
		...
		break;
	case 1:
		RTL_W32(tp, tp->imr_reg[0],
			LinkChg | RTL_R32(tp, tp->imr_reg[0]));
		break;
	}
...
static inline void rtl8125_enable_hw_interrupt(struct rtl8125_private *tp)
{
	switch (tp->HwCurrIsrVer) {
	case 2:
...
static inline void rtl8125_disable_hw_interrupt(struct rtl8125_private *tp)
{
...
static inline void rtl8125_switch_to_hw_interrupt(struct rtl8125_private *tp)
{
...
static inline void rtl8125_switch_to_timer_interrupt(struct rtl8125_private *tp)
{
...
static void rtl8125_irq_mask_and_ack(struct rtl8125_private *tp)
{
	...
	if (tp->HwCurrIsrVer == 2) {
		...
		if (tp->DASH) {
			if (tp->dash_printer_enabled) {
				RTL_W32(tp, tp->isr_reg[0],
					RTL_R32(tp, tp->isr_reg[0]) &
						~(ISRIMR_DASH_INTR_EN |
		...
		if (R8125_MULTI_RX_Q(tp)) {
			int i;
			for (i = 1; i < tp->num_rx_rings; i++)
				RTL_W16(tp, tp->isr_reg[i],
					RTL_R16(tp, tp->isr_reg[i]));
		}
	}
}

static void rtl8125_nic_reset(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	int i;
...
static void rtl8125_hw_set_interrupt_type(struct rtl8125_private *tp,
					  u8 isr_ver)
{
	u8 tmp;
...
static void rtl8125_hw_clear_timer_int(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static void rtl8125_hw_clear_int_miti(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	int i;
...
void rtl8125_hw_set_timer_int_8125(struct rtl8125_private *tp, u32 message_id,
...
static unsigned int rtl8125_xmii_reset_pending(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	unsigned int retval;
...
static unsigned int rtl8125_xmii_link_ok(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	unsigned int retval;
...
static int rtl8125_wait_phy_reset_complete(struct rtl8125_private *tp)
{
	int i, val;
...
static void rtl8125_xmii_reset_enable(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	...
	rtl8125_mdio_write(tp, 0x1f, 0x0000);
	rtl8125_mdio_write(tp, MII_ADVERTISE,
			   rtl8125_mdio_read(tp, MII_ADVERTISE) &
				   ~(ADVERTISE_10HALF | ADVERTISE_10FULL |
				     ADVERTISE_100HALF | ADVERTISE_100FULL));
	rtl8125_mdio_write(tp, MII_CTRL1000,
			   rtl8125_mdio_read(tp, MII_CTRL1000) &
				   ~(ADVERTISE_1000HALF | ADVERTISE_1000FULL));
	mdio_direct_write_phy_ocp(tp, 0xA5D4,
				  mdio_direct_read_phy_ocp(tp, 0xA5D4) &
...
static void rtl8125_issue_offset_99_event(struct rtl8125_private *tp)
{
	...
	case CFG_METHOD_6:
	case CFG_METHOD_7:
		rtl8125_mac_ocp_write(tp, 0xE09A,
				      rtl8125_mac_ocp_read(tp, 0xE09A) | BIT_0);
		break;
	}
}

#ifdef ENABLE_DASH_SUPPORT
static void NICChkTypeEnableDashInterrupt(struct rtl8125_private *tp)
{
	if (tp->DASH) {
		//
...
static int rtl8125_enable_eee_plus(struct rtl8125_private *tp)
{
	...
	case CFG_METHOD_6:
	case CFG_METHOD_7:
		rtl8125_mac_ocp_write(tp, 0xE080,
				      rtl8125_mac_ocp_read(tp, 0xE080) | BIT_1);
		break;
...
static int rtl8125_disable_eee_plus(struct rtl8125_private *tp)
{
	...
	case CFG_METHOD_7:
		rtl8125_mac_ocp_write(
			tp, 0xE080, rtl8125_mac_ocp_read(tp, 0xE080) & ~BIT_1);
		break;
...
static void rtl8125_link_on_patch(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);

	rtl8125_hw_config(dev);

	if ((tp->mcfg == CFG_METHOD_2) && netif_running(dev)) {
		if (RTL_R16(tp, PHYstatus) & FullDup)
			RTL_W32(tp, TxConfig,
				(RTL_R32(tp, TxConfig) | (BIT_24 | BIT_25)) &
					~BIT_19);
		else
			RTL_W32(tp, TxConfig,
				(RTL_R32(tp, TxConfig) | BIT_25) &
					~(BIT_19 | BIT_24));
	}
...
static void rtl8125_link_down_patch(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static void _rtl8125_check_link_status(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static void rtl8125_check_link_status(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static void rtl8125_link_option_giga(u8 *aut, u32 *spd, u8 *dup, u32 *adv)
{
...
static void rtl8125_link_option(u8 *aut, u32 *spd, u8 *dup, u32 *adv)
{
...
static void rtl8125_disable_ocp_phy_power_saving(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	u16 val;
...
static void rtl8125_wait_ll_share_fifo_ready(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	int i;
...
static void rtl8125_disable_pci_offset_99(struct rtl8125_private *tp)
{
	switch (tp->mcfg) {
	case CFG_METHOD_2:
	case CFG_METHOD_3:
	...
	case CFG_METHOD_6:
	case CFG_METHOD_7:
		rtl8125_mac_ocp_write(tp, 0xE032,
				      rtl8125_mac_ocp_read(tp, 0xE032) &
					      ~(BIT_0 | BIT_1));
		break;
	}
...
static void rtl8125_enable_pci_offset_99(struct rtl8125_private *tp)
{
	u32 csi_tmp;
...
static void rtl8125_init_pci_offset_99(struct rtl8125_private *tp)
{
	u32 csi_tmp;
...
static void rtl8125_disable_pci_offset_180(struct rtl8125_private *tp)
{
	u32 csi_tmp;
...
static void rtl8125_enable_pci_offset_180(struct rtl8125_private *tp)
{
	u32 csi_tmp;
...
static void rtl8125_init_pci_offset_180(struct rtl8125_private *tp)
{
...
static void rtl8125_set_pci_99_180_exit_driver_para(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static void rtl8125_enable_cfg9346_write(struct rtl8125_private *tp)
{
	RTL_W8(tp, Cfg9346, RTL_R8(tp, Cfg9346) | Cfg9346_Unlock);
}

static void rtl8125_disable_cfg9346_write(struct rtl8125_private *tp)
{
	RTL_W8(tp, Cfg9346, RTL_R8(tp, Cfg9346) & ~Cfg9346_Unlock);
}

static void rtl8125_enable_exit_l1_mask(struct rtl8125_private *tp)
{
	//(1)ERI(0xD4)(OCP 0xC0AC).bit[7:12]=6'b111111, L1 Mask
	SetMcuAccessRegBit(tp, 0xC0AC,
			   (BIT_7 | BIT_8 | BIT_9 | BIT_10 | BIT_11 | BIT_12));
}

static void rtl8125_disable_exit_l1_mask(struct rtl8125_private *tp)
{
	//(1)ERI(0xD4)(OCP 0xC0AC).bit[7:12]=6'b000000, L1 Mask
	ClearMcuAccessRegBit(
		tp, 0xC0AC, (BIT_7 | BIT_8 | BIT_9 | BIT_10 | BIT_11 | BIT_12));
}

static void rtl8125_enable_extend_tally_couter(struct rtl8125_private *tp)
{
...
static void rtl8125_disable_extend_tally_couter(struct rtl8125_private *tp)
{
	switch (tp->HwSuppExtendTallyCounterVer) {
	case 1:
...
static void rtl8125_hw_d3_para(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static void rtl8125_enable_magic_packet(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);

	switch (tp->HwSuppMagicPktVer) {
	case WAKEUP_MAGIC_PACKET_V3:
		rtl8125_mac_ocp_write(tp, 0xC0B6,
				      rtl8125_mac_ocp_read(tp, 0xC0B6) | BIT_0);
		break;
	}
}
static void rtl8125_disable_magic_packet(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	...
	case WAKEUP_MAGIC_PACKET_V3:
		rtl8125_mac_ocp_write(
			tp, 0xC0B6, rtl8125_mac_ocp_read(tp, 0xC0B6) & ~BIT_0);
		break;
	}
}

static void rtl8125_enable_linkchg_wakeup(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	...
	case 3:
		...
		ClearAndSetMcuAccessRegBit(tp, 0xE0C6, (BIT_5 | BIT_3 | BIT_2),
					   (BIT_4 | BIT_1 | BIT_0));
		break;
	}
}

static void rtl8125_disable_linkchg_wakeup(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	...
	case 3:
		RTL_W8(tp, Config3, RTL_R8(tp, Config3) & ~LinkUp);
		ClearMcuAccessRegBit(tp, 0xE0C6,
...
static u32 rtl8125_get_hw_wol(struct rtl8125_private *tp)
{
	u8 options;
	u32 csi_tmp;
	u32 wol_opts = 0;
...
static void rtl8125_disable_d0_speedup(struct rtl8125_private *tp)
{
	if (FALSE == HW_SUPPORT_D0_SPEED_UP(tp))
		return;
...
static void rtl8125_set_hw_wol(struct net_device *dev, u32 wolopts)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	int i, tmp;
	static struct {
...
static void rtl8125_phy_restart_nway(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static void rtl8125_phy_setup_force_mode(struct net_device *dev, u32 speed,
					 u8 duplex)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static void rtl8125_set_pci_pme(struct rtl8125_private *tp, int set)
{
	struct pci_dev *pdev = tp->pci_dev;
	u16 pmc;
...
static void rtl8125_set_wol_link_speed(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	int auto_nego;
	int giga_ctrl;
...
static bool rtl8125_keep_wol_link_speed(struct net_device *dev, u8 from_suspend)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static void rtl8125_powerdown_pll(struct net_device *dev, u8 from_suspend)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static void rtl8125_get_wol(struct net_device *dev, struct ethtool_wolinfo *wol)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	u8 options;
...
static int rtl8125_set_wol(struct net_device *dev, struct ethtool_wolinfo *wol)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static void rtl8125_get_drvinfo(struct net_device *dev,
...
static int rtl8125_get_regs_len(struct net_device *dev)
{
	return R8125_REGS_DUMP_SIZE;
}
#endif //LINUX_VERSION_CODE > KERNEL_VERSION(2,4,22)

static void rtl8125_set_d0_speedup_speed(struct rtl8125_private *tp)
{
	if (FALSE == HW_SUPPORT_D0_SPEED_UP(tp))
		return;
...
static int rtl8125_set_speed_xmii(struct net_device *dev, u8 autoneg, u32 speed,
				  u8 duplex, u32 adv)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	int auto_nego = 0;
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(2, 4, 22)
static int rtl8125_set_settings(struct net_device *dev,
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(3, 3, 0)
static u32 rtl8125_get_tx_csum(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	u32 ret;
...
static u32 rtl8125_get_rx_csum(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	u32 ret;
...
static int rtl8125_set_tx_csum(struct net_device *dev, u32 data)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static int rtl8125_set_rx_csum(struct net_device *dev, u32 data)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static u32 rtl8125_rx_desc_opts1(struct rtl8125_private *tp,
				 struct RxDesc *desc)
{
...
static u32 rtl8125_rx_desc_opts2(struct rtl8125_private *tp,
				 struct RxDesc *desc)
{
	if (tp->InitRxDescType == RX_DESC_RING_TYPE_3)
...
static void rtl8125_clear_rx_desc_opts2(struct rtl8125_private *tp,
					struct RxDesc *desc)
{
	if (tp->InitRxDescType == RX_DESC_RING_TYPE_3)
...
static inline u32 rtl8125_tx_vlan_tag(struct rtl8125_private *tp,
				      struct sk_buff *skb)
{
#if LINUX_VERSION_CODE < KERNEL_VERSION(3, 0, 0)
...
static void rtl8125_vlan_rx_register(struct net_device *dev,
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 22)
static void rtl8125_vlan_rx_kill_vid(struct net_device *dev, unsigned short vid)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static int rtl8125_rx_vlan_skb(struct rtl8125_private *tp, struct RxDesc *desc,
			       struct sk_buff *skb)
{
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(2, 4, 22)
static int rtl8125_get_settings(struct net_device *dev,
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 6, 0)
				struct ethtool_cmd *cmd
#else
				struct ethtool_link_ksettings *cmd
				  ...
				  struct ethtool_pauseparam *pause)
{
	...
	if (tp->fcpause != newfc) {
		...
		rtl8125_set_speed(dev, tp->autoneg, tp->speed, tp->duplex,
				  tp->advertising);
	}
...
static u32 rtl8125_get_msglevel(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static void rtl8125_set_msglevel(struct net_device *dev, u32 value)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static void rtl8125_set_ring_size(struct rtl8125_private *tp, u32 rx, u32 tx)
{
	int i;
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 17, 0)
static void rtl8125_get_ringparam(
	struct net_device *dev, struct ethtool_ringparam *ring,
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 17, 0)
static int rtl8125_set_ringparam(
	struct net_device *dev, struct ethtool_ringparam *ring,
	__maybe_unused struct kernel_ethtool_ringparam *kernel_param,
	__maybe_unused struct netlink_ext_ack *extack)
#else
...
#endif
{
	...
	new_tx_count = clamp_t(u32, ring->tx_pending, MIN_NUM_TX_DESC,
			       MAX_NUM_TX_DESC);

	new_rx_count = clamp_t(u32, ring->rx_pending, MIN_NUM_RX_DESC,
			       MAX_NUM_RX_DESC);
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(2, 4, 22)
static void rtl8125_get_ethtool_stats(struct net_device *dev,
...
static void rtl8125_get_strings(struct net_device *dev, u32 stringset, u8 *data)
{
	switch (stringset) {
	case ETH_SS_STATS:
...
static int rtl8125_enable_eee(struct rtl8125_private *tp)
{
	...
	case CFG_METHOD_6:
		...
		ClearAndSetEthPhyOcpBit(
			tp, 0xA5D0, MDIO_EEE_100TX | MDIO_EEE_1000T, eee_adv_t);
...
static u32 rtl8125_tx_lpi_timer_to_us(struct rtl8125_private *tp,
				      u32 tx_lpi_timer)
{
	u32 to_us;
	u16 status;
			       ...
			       struct ethtool_eee *edata)
{
	...
	if (!HW_HAS_WRITE_PHY_MCU_RAM_CODE(tp) || tp->DASH)
		return -EOPNOTSUPP;
	...
	if (tp->autoneg != AUTONEG_ENABLE) {
		dev_printk(KERN_WARNING, tp_to_dev(tp),
	...
	if (edata->tx_lpi_enabled) {
		if (edata->tx_lpi_timer > tp->max_jumbo_frame_size ||
			...
			dev_printk(KERN_WARNING, tp_to_dev(tp),
				   "Valid LPI timer range is %d to %d. \n",
	...
	if (edata->advertised & ~eee->supported) {
		dev_printk(KERN_WARNING, tp_to_dev(tp),
			   "EEE advertised %x must be a subset of support %x\n",
	...
	dev_printk(KERN_WARNING, tp_to_dev(tp),
		   "EEE tx_lpi_timer %x must be a subset of support %x\n",
...
static void rtl8125_print_mac_version(struct rtl8125_private *tp)
{
	int i;
...
static void rtl8125_tally_counter_addr_fill(struct rtl8125_private *tp)
{
	if (!tp->tally_paddr)
		return;
...
static void rtl8125_tally_counter_clear(struct rtl8125_private *tp)
{
	if (!tp->tally_paddr)
		return;
...
static void rtl8125_clear_phy_ups_reg(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static int rtl8125_is_ups_resume(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static void rtl8125_clear_ups_resume_bit(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	...
	if (tp->mcfg == CFG_METHOD_2 || tp->mcfg == CFG_METHOD_3 ||
	    tp->mcfg == CFG_METHOD_4 || tp->mcfg == CFG_METHOD_5 ||
	    tp->mcfg == CFG_METHOD_6 || tp->mcfg == CFG_METHOD_7)
		rtl8125_mac_ocp_write(tp, 0xD408,
				      rtl8125_mac_ocp_read(tp, 0xD408) &
					      ~(BIT_8));
}

static void rtl8125_wait_phy_ups_resume(struct net_device *dev, u16 PhyState)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	u16 TmpPhyState;
	int i = 0;
...
static void rtl8125_exit_oob(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	u16 data16;
...
static void rtl8125_switch_mac_mcu_ram_code_page(struct rtl8125_private *tp,
						 u16 page)
{
	u16 tmpUshort;
...
static void _rtl8125_write_mac_mcu_ram_code(struct rtl8125_private *tp,
...
static void
_rtl8125_write_mac_mcu_ram_code_with_page(struct rtl8125_private *tp,
					  const u16 *entry, u16 entry_cnt,
					  u16 page_size)
{
	u16 i;
...
static void rtl8125_write_mac_mcu_ram_code(struct rtl8125_private *tp,
					   const u16 *entry, u16 entry_cnt)
{
	...
	if (tp->MacMcuPageSize > 0)
		_rtl8125_write_mac_mcu_ram_code_with_page(tp, entry, entry_cnt,
							  tp->MacMcuPageSize);
	else
		_rtl8125_write_mac_mcu_ram_code(tp, entry, entry_cnt);
}

static void rtl8125_set_mac_mcu_8125a_1(struct net_device *dev)
{
	rtl8125_hw_disable_mac_mcu_bps(dev);
}

static void rtl8125_set_mac_mcu_8125a_2(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	static const u16 mcu_patch_code_8125a_2[] = {
		0xE010, 0xE012, 0xE022, 0xE024, 0xE029, 0xE02B, 0xE094, 0xE09D,
		0xE09F, 0xE0AA, 0xE0B5, 0xE0C6, 0xE0CC, 0xE0D1, 0xE0D6, 0xE0D8,
		0xC602, 0xBE00, 0x0000, 0xC60F, 0x73C4, 0x49B3, 0xF106, 0x73C2,
		0xC608, 0xB406, 0xC609, 0xFF80, 0xC605, 0xB406, 0xC605, 0xFF80,
		0x0544, 0x0568, 0xE906, 0xCDE8, 0xC602, 0xBE00, 0x0000, 0x48C1,
		0x48C2, 0x9C46, 0xC402, 0xBC00, 0x0A12, 0xC602, 0xBE00, 0x0EBA,
		0x1501, 0xF02A, 0x1500, 0xF15D, 0xC661, 0x75C8, 0x49D5, 0xF00A,
		0x49D6, 0xF008, 0x49D7, 0xF006, 0x49D8, 0xF004, 0x75D2, 0x49D9,
		0xF150, 0xC553, 0x77A0, 0x75C8, 0x4855, 0x4856, 0x4857, 0x4858,
		0x48DA, 0x48DB, 0x49FE, 0xF002, 0x485A, 0x49FF, 0xF002, 0x485B,
		0x9DC8, 0x75D2, 0x4859, 0x9DD2, 0xC643, 0x75C0, 0x49D4, 0xF033,
		0x49D0, 0xF137, 0xE030, 0xC63A, 0x75C8, 0x49D5, 0xF00E, 0x49D6,
		0xF00C, 0x49D7, 0xF00A, 0x49D8, 0xF008, 0x75D2, 0x49D9, 0xF005,
		0xC62E, 0x75C0, 0x49D7, 0xF125, 0xC528, 0x77A0, 0xC627, 0x75C8,
		0x4855, 0x4856, 0x4857, 0x4858, 0x48DA, 0x48DB, 0x49FE, 0xF002,
		0x485A, 0x49FF, 0xF002, 0x485B, 0x9DC8, 0x75D2, 0x4859, 0x9DD2,
		0xC616, 0x75C0, 0x4857, 0x9DC0, 0xC613, 0x75C0, 0x49DA, 0xF003,
		0x49D0, 0xF107, 0xC60B, 0xC50E, 0x48D9, 0x9DC0, 0x4859, 0x9DC0,
		0xC608, 0xC702, 0xBF00, 0x3AE0, 0xE860, 0xB400, 0xB5D4, 0xE908,
		0xE86C, 0x1200, 0xC409, 0x6780, 0x48F1, 0x8F80, 0xC404, 0xC602,
		0xBE00, 0x10AA, 0xC010, 0xEA7C, 0xC602, 0xBE00, 0x0000, 0x740A,
		0x4846, 0x4847, 0x9C0A, 0xC607, 0x74C0, 0x48C6, 0x9CC0, 0xC602,
		0xBE00, 0x13FE, 0xE054, 0x72CA, 0x4826, 0x4827, 0x9ACA, 0xC607,
		0x72C0, 0x48A6, 0x9AC0, 0xC602, 0xBE00, 0x07DC, 0xE054, 0xC60F,
		0x74C4, 0x49CC, 0xF109, 0xC60C, 0x74CA, 0x48C7, 0x9CCA, 0xC609,
		0x74C0, 0x4846, 0x9CC0, 0xC602, 0xBE00, 0x2480, 0xE092, 0xE0C0,
		0xE054, 0x7420, 0x48C0, 0x9C20, 0x7444, 0xC602, 0xBE00, 0x12F8,
		0x1BFF, 0x46EB, 0x1BFF, 0xC102, 0xB900, 0x0D5A, 0x1BFF, 0x46EB,
		0x1BFF, 0xC102, 0xB900, 0x0E2A, 0xC602, 0xBE00, 0x0000, 0xC602,
		...
		0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
		0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
		0x0000, 0x0000, 0x0000, 0x0000, 0x6486, 0x0B15, 0x090E, 0x1139
	};

	rtl8125_hw_disable_mac_mcu_bps(dev);

	rtl8125_write_mac_mcu_ram_code(tp, mcu_patch_code_8125a_2,
...
static void rtl8125_set_mac_mcu_8125b_1(struct net_device *dev)
{
	rtl8125_hw_disable_mac_mcu_bps(dev);
}

static void rtl8125_set_mac_mcu_8125b_2(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	static const u16 mcu_patch_code_8125b_2[] = {
		0xE010, 0xE01B, 0xE026, 0xE037, 0xE03D, 0xE057, 0xE05B, 0xE060,
		0xE062, 0xE064, 0xE066, 0xE068, 0xE06A, 0xE06C, 0xE06E, 0xE070,
		0x740A, 0x4846, 0x4847, 0x9C0A, 0xC607, 0x74C0, 0x48C6, 0x9CC0,
		0xC602, 0xBE00, 0x13F0, 0xE054, 0x72CA, 0x4826, 0x4827, 0x9ACA,
		0xC607, 0x72C0, 0x48A6, 0x9AC0, 0xC602, 0xBE00, 0x081C, 0xE054,
		0xC60F, 0x74C4, 0x49CC, 0xF109, 0xC60C, 0x74CA, 0x48C7, 0x9CCA,
		0xC609, 0x74C0, 0x4846, 0x9CC0, 0xC602, 0xBE00, 0x2494, 0xE092,
		0xE0C0, 0xE054, 0x7420, 0x48C0, 0x9C20, 0x7444, 0xC602, 0xBE00,
		0x12DC, 0x733A, 0x21B5, 0x25BC, 0x1304, 0xF111, 0x1B12, 0x1D2A,
		0x3168, 0x3ADA, 0x31AB, 0x1A00, 0x9AC0, 0x1300, 0xF1FB, 0x7620,
		0x236E, 0x276F, 0x1A3C, 0x22A1, 0x41B5, 0x9EE2, 0x76E4, 0x486F,
		0x9EE4, 0xC602, 0xBE00, 0x4A26, 0x733A, 0x49BB, 0xC602, 0xBE00,
		0x47A2, 0x48C1, 0x48C2, 0x9C46, 0xC402, 0xBC00, 0x0A52, 0xC602,
	...
	rtl8125_write_mac_mcu_ram_code(tp, mcu_patch_code_8125b_2,
...
static void rtl8125_hw_mac_mcu_config(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static void rtl8125_hw_init(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	u32 csi_tmp;
	...
	case CFG_METHOD_6:
	case CFG_METHOD_7:
		rtl8125_mac_ocp_write(tp, 0xD40A,
				      rtl8125_mac_ocp_read(tp, 0xD40A) &
					      ~(BIT_4));
		break;
	}
	...
	if (tp->rtl_fw && !tp->resume_not_chg_speed &&
	    !(HW_DASH_SUPPORT_TYPE_3(tp) && tp->HwPkgDet == 0x06))
		rtl8125_apply_firmware(tp);
#endif
}

static void rtl8125_hw_ephy_config(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static int rtl8125_check_hw_phy_mcu_code_ver(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
#ifndef ENABLE_USE_FIRMWARE_FILE
static void rtl8125_write_hw_phy_mcu_code_ver(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static void rtl8125_acquire_phy_mcu_patch_key_lock(struct rtl8125_private *tp)
{
	u16 PatchKey;
...
static void rtl8125_release_phy_mcu_patch_key_lock(struct rtl8125_private *tp)
{
	switch (tp->mcfg) {
	case CFG_METHOD_2:
	case CFG_METHOD_3:
...
static void rtl8125_set_phy_mcu_ram_code(struct net_device *dev,
...
static void rtl8125_enable_phy_disable_mode(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static void rtl8125_disable_phy_disable_mode(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static void rtl8125_set_hw_phy_before_init_phy_mcu(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	u16 PhyRegValue;
	...
	case CFG_METHOD_4:
		...
		ClearAndSetEthPhyOcpBit(tp, 0xBD4E, BIT_11 | BIT_10, BIT_11);
		ClearAndSetEthPhyOcpBit(tp, 0xBF46,
...
static void rtl8125_real_set_phy_mcu_8125a_1(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static void rtl8125_set_phy_mcu_8125a_1(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static void rtl8125_real_set_phy_mcu_8125a_2(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static void rtl8125_set_phy_mcu_8125a_2(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static const u16 phy_mcu_ram_code_8125b_2[] = {
	...
	0x1800, 0xa438, 0x8079, 0xa438, 0x1800, 0xa438, 0x807e, 0xa438, 0xd093,
	...
	0xa438, 0x1800, 0xa438, 0x0916, 0xa438, 0x1000, 0xa438, 0x14c5, 0xa438,
	0xd703, 0xa438, 0x3181, 0xa438, 0x8077, 0xa438, 0x60ad, 0xa438, 0x1000,
	0xa438, 0x135c, 0xa438, 0xd703, 0xa438, 0x5fba, 0xa438, 0x1800, 0xa438,
	0x0cc7, 0xa438, 0xd096, 0xa438, 0xd1a9, 0xa438, 0xd503, 0xa438, 0x1800,
	0xa438, 0x0c94, 0xa438, 0xa802, 0xa438, 0xa301, 0xa438, 0xa801, 0xa438,
	0xc004, 0xa438, 0xd710, 0xa438, 0x4000, 0xa438, 0x1800, 0xa438, 0x1e79,
	0xa436, 0xA026, 0xa438, 0x1e78, 0xa436, 0xA024, 0xa438, 0x0c93, 0xa436,
	0xA022, 0xa438, 0x0cc5, 0xa436, 0xA020, 0xa438, 0x0915, 0xa436, 0xA006,
	0xa438, 0x020a, 0xa436, 0xA004, 0xa438, 0x1726, 0xa436, 0xA002, 0xa438,
	0x1542, 0xa436, 0xA000, 0xa438, 0x0fc7, 0xa436, 0xA008, 0xa438, 0xff00,
	0xa436, 0xA016, 0xa438, 0x0010, 0xa436, 0xA012, 0xa438, 0x0000, 0xa436,
	0xA014, 0xa438, 0x1800, 0xa438, 0x8010, 0xa438, 0x1800, 0xa438, 0x801d,
	0xa438, 0x1800, 0xa438, 0x802c, 0xa438, 0x1800, 0xa438, 0x802c, 0xa438,
	0x1800, 0xa438, 0x802c, 0xa438, 0x1800, 0xa438, 0x802c, 0xa438, 0x1800,
	0xa438, 0x802c, 0xa438, 0x1800, 0xa438, 0x802c, 0xa438, 0xd700, 0xa438,
	0x6090, 0xa438, 0x60d1, 0xa438, 0xc95c, 0xa438, 0xf007, 0xa438, 0x60b1,
	0xa438, 0xc95a, 0xa438, 0xf004, 0xa438, 0xc956, 0xa438, 0xf002, 0xa438,
	0xc94e, 0xa438, 0x1800, 0xa438, 0x00cd, 0xa438, 0xd700, 0xa438, 0x6090,
	0xa438, 0x60d1, 0xa438, 0xc95c, 0xa438, 0xf007, 0xa438, 0x60b1, 0xa438,
	0xc95a, 0xa438, 0xf004, 0xa438, 0xc956, 0xa438, 0xf002, 0xa438, 0xc94e,
	0xa438, 0x1000, 0xa438, 0x022a, 0xa438, 0x1800, 0xa438, 0x0132, 0xa436,
	0xa438, 0x52c8, 0xa438, 0xa131, 0xa438, 0x03af, 0xa438, 0x85a7, 0xa438,
	0xd480, 0xa438, 0x00bf, 0xa438, 0x8684, 0xa438, 0x0252, 0xa438, 0xa9bf,
	0xa438, 0x8687, 0xa438, 0x0252, 0xa438, 0xa9bf, 0xa438, 0x868a, 0xa438,
	0x0252, 0xa438, 0xa9bf, 0xa438, 0x868d, 0xa438, 0x0252, 0xa438, 0xa9ef,
	0xa438, 0x95fd, 0xa438, 0xfc04, 0xa438, 0xf0d1, 0xa438, 0x2af0, 0xa438,
	0xd12c, 0xa438, 0xf0d1, 0xa438, 0x44f0, 0xa438, 0xd146, 0xa438, 0xbf86,
	0xa438, 0xa102, 0xa438, 0x52c8, 0xa438, 0xbf86, 0xa438, 0xa102, 0xa438,
	0x52c8, 0xa438, 0xd101, 0xa438, 0xaf06, 0xa438, 0xa570, 0xa438, 0xce42,
	0xa436, 0xb818, 0xa438, 0x043d, 0xa436, 0xb81a, 0xa438, 0x06a3, 0xa436,
	0xb81c, 0xa438, 0xffff, 0xa436, 0xb81e, 0xa438, 0xffff, 0xa436, 0xb850,
	0xa438, 0xffff, 0xa436, 0xb852, 0xa438, 0xffff, 0xa436, 0xb878, 0xa438,
	0xffff, 0xa436, 0xb884, 0xa438, 0xffff, 0xa436, 0xb832, 0xa438, 0x0003,
	0xa436, 0x0000, 0xa438, 0x0000, 0xa436, 0xB82E, 0xa438, 0x0000, 0xa436,
	0x8024, 0xa438, 0x0000, 0xb820, 0x0000, 0xa436, 0x801E, 0xa438, 0x0020,
	0xFFFF, 0xFFFF
};

static void rtl8125_real_set_phy_mcu_8125b_1(struct net_device *dev)
{
	rtl8125_set_phy_mcu_ram_code(dev, phy_mcu_ram_code_8125b_1,
				     ARRAY_SIZE(phy_mcu_ram_code_8125b_1));
}

static void rtl8125_set_phy_mcu_8125b_1(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static void rtl8125_real_set_phy_mcu_8125b_2(struct net_device *dev)
{
	rtl8125_set_phy_mcu_ram_code(dev, phy_mcu_ram_code_8125b_2,
				     ARRAY_SIZE(phy_mcu_ram_code_8125b_2));
}

static void rtl8125_set_phy_mcu_8125b_2(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static void rtl8125_init_hw_phy_mcu(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	...
	if (tp->NotWrRamCodeToMicroP == TRUE)
		return;
	if (rtl8125_check_hw_phy_mcu_code_ver(dev))
		return;

	if (HW_SUPPORT_CHECK_PHY_DISABLE_MODE(tp) &&
	    rtl8125_is_in_phy_disable_mode(dev))
...
static void rtl8125_enable_phy_aldps(struct rtl8125_private *tp)
{
	//enable aldps
...
static void rtl8125_hw_phy_config_8125a_1(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);

	ClearAndSetEthPhyOcpBit(tp, 0xAD40, 0x03FF, 0x84);

	SetEthPhyOcpBit(tp, 0xAD4E, BIT_4);
	ClearAndSetEthPhyOcpBit(tp, 0xAD16, 0x03FF, 0x0006);
	ClearAndSetEthPhyOcpBit(tp, 0xAD32, 0x003F, 0x0006);
	ClearEthPhyOcpBit(tp, 0xAC08, BIT_12);
	ClearEthPhyOcpBit(tp, 0xAC08, BIT_8);
	ClearAndSetEthPhyOcpBit(tp, 0xAC8A, BIT_15 | BIT_14 | BIT_13 | BIT_12,
	...
	ClearAndSetEthPhyOcpBit(tp, 0xA438, 0xFF00, 0xC400);
	mdio_direct_write_phy_ocp(tp, 0xA436, 0x80EB);
	ClearAndSetEthPhyOcpBit(tp, 0xA438, 0x0700, 0x0300);
	mdio_direct_write_phy_ocp(tp, 0xA436, 0x80F8);
	ClearAndSetEthPhyOcpBit(tp, 0xA438, 0xFF00, 0x1C00);
	mdio_direct_write_phy_ocp(tp, 0xA436, 0x80F1);
	ClearAndSetEthPhyOcpBit(tp, 0xA438, 0xFF00, 0x3000);

	mdio_direct_write_phy_ocp(tp, 0xA436, 0x80FE);
	ClearAndSetEthPhyOcpBit(tp, 0xA438, 0xFF00, 0xA500);
	mdio_direct_write_phy_ocp(tp, 0xA436, 0x8102);
	ClearAndSetEthPhyOcpBit(tp, 0xA438, 0xFF00, 0x5000);
	mdio_direct_write_phy_ocp(tp, 0xA436, 0x8105);
	ClearAndSetEthPhyOcpBit(tp, 0xA438, 0xFF00, 0x3300);
	mdio_direct_write_phy_ocp(tp, 0xA436, 0x8100);
	ClearAndSetEthPhyOcpBit(tp, 0xA438, 0xFF00, 0x7000);
	mdio_direct_write_phy_ocp(tp, 0xA436, 0x8104);
	ClearAndSetEthPhyOcpBit(tp, 0xA438, 0xFF00, 0xF000);
	mdio_direct_write_phy_ocp(tp, 0xA436, 0x8106);
	ClearAndSetEthPhyOcpBit(tp, 0xA438, 0xFF00, 0x6500);
	mdio_direct_write_phy_ocp(tp, 0xA436, 0x80DC);
	ClearAndSetEthPhyOcpBit(tp, 0xA438, 0xFF00, 0xED00);
	...
	ClearAndSetEthPhyOcpBit(tp, 0xBF06, 0x003F, 0x38);
	...
	ClearAndSetEthPhyOcpBit(tp, 0xBF0A, BIT_11 | BIT_10 | BIT_9,
				BIT_11 | BIT_9);
...
static void rtl8125_hw_phy_config_8125a_2(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);

	SetEthPhyOcpBit(tp, 0xAD4E, BIT_4);
	ClearAndSetEthPhyOcpBit(tp, 0xAD16, 0x03FF, 0x03FF);
	ClearAndSetEthPhyOcpBit(tp, 0xAD32, 0x003F, 0x0006);
	ClearEthPhyOcpBit(tp, 0xAC08, BIT_12);
	ClearEthPhyOcpBit(tp, 0xAC08, BIT_8);
	ClearAndSetEthPhyOcpBit(tp, 0xACC0, BIT_1 | BIT_0, BIT_1);
	ClearAndSetEthPhyOcpBit(tp, 0xAD40, BIT_7 | BIT_6 | BIT_5, BIT_6);
	ClearAndSetEthPhyOcpBit(tp, 0xAD40, BIT_2 | BIT_1 | BIT_0, BIT_2);
	ClearEthPhyOcpBit(tp, 0xAC14, BIT_7);
	ClearEthPhyOcpBit(tp, 0xAC80, BIT_9 | BIT_8);
	ClearAndSetEthPhyOcpBit(tp, 0xAC5E, BIT_2 | BIT_1 | BIT_0, BIT_1);
	mdio_direct_write_phy_ocp(tp, 0xAD4C, 0x00A8);
	mdio_direct_write_phy_ocp(tp, 0xAC5C, 0x01FF);
	ClearAndSetEthPhyOcpBit(tp, 0xAC8A, BIT_7 | BIT_6 | BIT_5 | BIT_4,
				BIT_5 | BIT_4);
	mdio_direct_write_phy_ocp(tp, 0xB87C, 0x8157);
	ClearAndSetEthPhyOcpBit(tp, 0xB87E, 0xFF00, 0x0500);
	mdio_direct_write_phy_ocp(tp, 0xB87C, 0x8159);
	ClearAndSetEthPhyOcpBit(tp, 0xB87E, 0xFF00, 0x0700);
	...
	SetEthPhyOcpBit(tp, 0xA438, BIT_8);
	ClearAndSetEthPhyOcpBit(tp, 0xB54C, 0xFF00, 0xDB00);
	...
	if (tp->RequirePhyMdiSwapPatch) {
		...
		ClearAndSetEthPhyOcpBit(tp, 0xD068, 0x0007, 0x0001);
		ClearAndSetEthPhyOcpBit(tp, 0xD068, 0x0018, 0x0000);
		adccal_offset_p0 = mdio_direct_read_phy_ocp(tp, 0xD06A);
		adccal_offset_p0 &= 0x07FF;
		ClearAndSetEthPhyOcpBit(tp, 0xD068, 0x0018, 0x0008);
		adccal_offset_p1 = mdio_direct_read_phy_ocp(tp, 0xD06A);
		adccal_offset_p1 &= 0x07FF;
		ClearAndSetEthPhyOcpBit(tp, 0xD068, 0x0018, 0x0010);
		adccal_offset_p2 = mdio_direct_read_phy_ocp(tp, 0xD06A);
		adccal_offset_p2 &= 0x07FF;
		ClearAndSetEthPhyOcpBit(tp, 0xD068, 0x0018, 0x0018);
		adccal_offset_p3 = mdio_direct_read_phy_ocp(tp, 0xD06A);
		adccal_offset_p3 &= 0x07FF;

		ClearAndSetEthPhyOcpBit(tp, 0xD068, 0x0018, 0x0000);
		ClearAndSetEthPhyOcpBit(tp, 0xD06A, 0x07FF, adccal_offset_p3);
		ClearAndSetEthPhyOcpBit(tp, 0xD068, 0x0018, 0x0008);
		ClearAndSetEthPhyOcpBit(tp, 0xD06A, 0x07FF, adccal_offset_p2);
		ClearAndSetEthPhyOcpBit(tp, 0xD068, 0x0018, 0x0010);
		ClearAndSetEthPhyOcpBit(tp, 0xD06A, 0x07FF, adccal_offset_p1);
		ClearAndSetEthPhyOcpBit(tp, 0xD068, 0x0018, 0x0018);
		ClearAndSetEthPhyOcpBit(tp, 0xD06A, 0x07FF, adccal_offset_p0);
		...
		ClearAndSetEthPhyOcpBit(tp, 0xBD5A, 0x001F,
					rg_lpf_cap_xg_p3 >> 8);
		ClearAndSetEthPhyOcpBit(tp, 0xBD5A, 0x1F00,
					rg_lpf_cap_xg_p2 << 8);
		ClearAndSetEthPhyOcpBit(tp, 0xBD5C, 0x001F,
					rg_lpf_cap_xg_p1 >> 8);
		ClearAndSetEthPhyOcpBit(tp, 0xBD5C, 0x1F00,
					rg_lpf_cap_xg_p0 << 8);
		ClearAndSetEthPhyOcpBit(tp, 0xBC18, 0x001F, rg_lpf_cap_p3 >> 8);
		ClearAndSetEthPhyOcpBit(tp, 0xBC18, 0x1F00, rg_lpf_cap_p2 << 8);
		ClearAndSetEthPhyOcpBit(tp, 0xBC1A, 0x001F, rg_lpf_cap_p1 >> 8);
		ClearAndSetEthPhyOcpBit(tp, 0xBC1A, 0x1F00, rg_lpf_cap_p0 << 8);
	}
...
static void rtl8125_hw_phy_config_8125b_1(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	...
	if (HW_HAS_WRITE_PHY_MCU_RAM_CODE(tp)) {
		...
		ClearAndSetEthPhyOcpBit(tp, 0xA438, 0xFF00, 0x0400);
	}
	...
	ClearAndSetEthPhyOcpBit(tp, 0xACDA, 0xFF00, 0xFF00);
	ClearAndSetEthPhyOcpBit(tp, 0xACDE, 0xF000, 0xF000);
	...
	mdio_direct_write_phy_ocp(tp, 0xA436, 0x80DC);
	ClearAndSetEthPhyOcpBit(tp, 0xA438, 0xFF00, 0x1000);
	...
	ClearAndSetEthPhyOcpBit(tp, 0xA438, 0xFF00, 0x6C00);
	...
	ClearAndSetEthPhyOcpBit(tp, 0xA438, 0xFF00, 0x9F00);
	...
	ClearAndSetEthPhyOcpBit(tp, 0xA438, 0xFF00, 0x8000);
	...
	ClearAndSetEthPhyOcpBit(tp, 0xA438, 0xFF00, 0xC000);
...
static void rtl8125_hw_phy_config_8125b_2(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);

	SetEthPhyOcpBit(tp, 0xA442, BIT_11);

	ClearAndSetEthPhyOcpBit(tp, 0xAC46, 0x00F0, 0x0090);
	ClearAndSetEthPhyOcpBit(tp, 0xAD30, 0x0003, 0x0001);
	...
	ClearAndSetEthPhyOcpBit(tp, 0xB87E,
				BIT_15 | BIT_14 | BIT_13 | BIT_12 | BIT_11 |
					BIT_10 | BIT_9 | BIT_8,
				BIT_11);

	ClearAndSetEthPhyOcpBit(tp, 0xbf00, 0xE000, 0xA000);
	ClearAndSetEthPhyOcpBit(tp, 0xbf46, 0x0F00, 0x0300);
	...
	ClearAndSetEthPhyOcpBit(tp, 0xBF84, BIT_15 | BIT_14 | BIT_13,
				BIT_15 | BIT_13);

	mdio_direct_write_phy_ocp(tp, 0xA436, 0x8170);
	ClearAndSetEthPhyOcpBit(tp, 0xA438, BIT_13 | BIT_10 | BIT_9 | BIT_8,
...
static void rtl8125_hw_phy_config(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static void rtl8125_up(struct net_device *dev)
{
	rtl8125_hw_init(dev);
	rtl8125_hw_reset(dev);
 ...
 */
static void rtl8125_netpoll(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	int i;
...
static void rtl8125_get_bios_setting(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static void rtl8125_set_bios_setting(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static void rtl8125_setup_mqs_reg(struct rtl8125_private *tp)
{
	int i;
...
static void rtl8125_init_software_variable(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	struct pci_dev *pdev = tp->pci_dev;
	...
	if (tp->DASH) {
		if (HW_DASH_SUPPORT_TYPE_3(tp)) {
			...
			//map CMAC IO space
			CmacMemPhysAddress =
				rtl8125_csi_other_fun_read(tp, 0, 0x18);
			if (!(CmacMemPhysAddress & BIT_0)) {
				if (CmacMemPhysAddress & BIT_2)
					CmacMemPhysAddress |=
						(u64)rtl8125_csi_other_fun_read(
							tp, 0, 0x1C)
						<< 32;

				CmacMemPhysAddress &= 0xFFFFFFF0;
				/* ioremap MMIO region */
				cmac_ioaddr = ioremap(CmacMemPhysAddress,
						      R8125_REGS_SIZE);
			}
	...
	if (aspm) {
		...
		case CFG_METHOD_7:
			tp->org_pci_offset_99 =
				rtl8125_csi_fun0_read_byte(tp, 0x99);
		...
		case CFG_METHOD_6:
			tp->org_pci_offset_180 =
				rtl8125_csi_fun0_read_byte(tp, 0x264);
			break;
		case CFG_METHOD_4:
		case CFG_METHOD_5:
		case CFG_METHOD_7:
			tp->org_pci_offset_180 =
				rtl8125_csi_fun0_read_byte(tp, 0x214);
			break;
		}
	}
	...
	if (tp->mcfg == CFG_METHOD_6 || tp->mcfg == CFG_METHOD_7)
		rtl8125_link_option_giga((u8 *)&autoneg_mode,
					 (u32 *)&speed_mode, (u8 *)&duplex_mode,
					 (u32 *)&advertising_mode);
	else
		rtl8125_link_option((u8 *)&autoneg_mode, (u32 *)&speed_mode,
				    (u8 *)&duplex_mode,
				    (u32 *)&advertising_mode);
	...
	if (tp->mcfg != CFG_METHOD_DEFAULT) {
		...
		eee->advertised = mmd_eee_adv_to_ethtool_adv_t(MDIO_EEE_1000T |
							       MDIO_EEE_100TX);
...
static void rtl8125_release_board(struct pci_dev *pdev, struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	void __iomem *ioaddr = tp->mmio_addr;
...
static int rtl8125_get_mac_address(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	int i;
...
static int rtl8125_do_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static int __devinit rtl8125_init_board(struct pci_dev *pdev,
					...
					void __iomem **ioaddr_out)
{
	...
	/* ioremap MMIO region */
	ioaddr =
		ioremap(pci_resource_start(pdev, 2), pci_resource_len(pdev, 2));
	if (ioaddr == NULL) {
#if LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 0)
		if (netif_msg_probe(tp))
...
static void rtl8125_esd_checker(struct rtl8125_private *tp)
{
	struct net_device *dev = tp->dev;
	struct pci_dev *pdev = tp->pci_dev;
	u8 cmd;
	...
	if (tp->HwPcieSNOffset > 0) {
		...
		if (pci_sn_l != tp->pci_cfg_space.pci_sn_l) {
			...
			rtl8125_csi_write(tp, tp->HwPcieSNOffset,
		...
		if (pci_sn_h != tp->pci_cfg_space.pci_sn_h) {
			...
			rtl8125_csi_write(tp, tp->HwPcieSNOffset + 4,
...
int rtl8125_enable_msix(struct rtl8125_private *tp)
{
	int i, nvecs = 0;
	...
	nvecs = pci_enable_msix_range(tp->pci_dev, msix_ent, tp->min_irq_nvecs,
				      tp->max_irq_nvecs);
	if (nvecs < 0)
		goto out;
...
void rtl8125_dump_msix_tbl(struct rtl8125_private *tp)
{
	...
	/* ioremap MMIO region */
	ioaddr = ioremap(pci_resource_start(tp->pci_dev, 4),
			 pci_resource_len(tp->pci_dev, 4));
	if (ioaddr) {
		int i = 0;
		for (i = 0; i < tp->irq_nvecs; i++) {
...
static int rtl8125_try_msi(struct rtl8125_private *tp)
{
	...
	if ((nvecs = pci_alloc_irq_vectors(pdev, tp->min_irq_nvecs,
					   tp->max_irq_nvecs, PCI_IRQ_MSIX)) >
	    0)
		msi |= RTL_FEATURE_MSIX;
	else if ((nvecs = pci_alloc_irq_vectors(pdev, 1, 1,
						PCI_IRQ_ALL_TYPES)) > 0 &&
...
static void rtl8125_disable_msi(struct pci_dev *pdev,
				struct rtl8125_private *tp)
{
#if defined(RTL_USE_NEW_INTR_API)
...
static void rtl8125_init_napi(struct rtl8125_private *tp)
{
	...
	for (i = 0; i < tp->irq_nvecs; i++) {
		...
		if (tp->features & RTL_FEATURE_MSIX && tp->HwCurrIsrVer == 2) {
...
static int rtl8125_set_real_num_queue(struct rtl8125_private *tp)
{
	int retval = 0;
...
static int __devinit rtl8125_init_one(struct pci_dev *pdev,
...
static void __devexit rtl8125_remove_one(struct pci_dev *pdev)
{
...
static void rtl8125_set_rxbufsize(struct rtl8125_private *tp,
				  struct net_device *dev)
{
...
static int rtl8125_alloc_irq(struct rtl8125_private *tp)
{
	...
	if (tp->features & RTL_FEATURE_MSIX && tp->HwCurrIsrVer == 2) {
		for (i = 0; i < tp->irq_nvecs; i++) {
			...
			r8125napi = &tp->r8125napi[i];
			snprintf(irq->name, len, "%s-%d", dev->name, i);
			rc = request_irq(irq->vector, irq->handler, irq_flags,
					 irq->name, r8125napi);
...
static int rtl8125_alloc_tx_desc(struct rtl8125_private *tp)
{
	...
	for (i = 0; i < tp->num_tx_rings; i++) {
		ring = &tp->tx_ring[i];
		ring->TxDescArray = dma_alloc_coherent(
			&pdev->dev, (ring->num_tx_desc * sizeof(struct TxDesc)),
...
static int rtl8125_alloc_rx_desc(struct rtl8125_private *tp)
{
	...
	for (i = 0; i < tp->num_rx_rings; i++) {
		ring = &tp->rx_ring[i];
		ring->RxDescArray = dma_alloc_coherent(
			&pdev->dev, (ring->num_rx_desc * tp->RxDescLength),
...
static int rtl8125_alloc_patch_mem(struct rtl8125_private *tp)
{
	...
	if (tp->RequireRduNonStopPatch) {
		tp->ShortPacketEmptyBuffer = dma_alloc_coherent(
			&pdev->dev, SHORT_PACKET_PADDING_BUF_SIZE,
...
static void rtl8125_free_patch_mem(struct rtl8125_private *tp)
{
	...
	if (tp->ShortPacketEmptyBuffer) {
		dma_free_coherent(&pdev->dev, SHORT_PACKET_PADDING_BUF_SIZE,
...
static void set_offset70F(struct rtl8125_private *tp, u8 setting)
{
	u32 csi_tmp;
...
static void set_offset79(struct rtl8125_private *tp, u8 setting)
{
...
void rtl8125_hw_set_rx_packet_filter(struct net_device *dev)
{
	...
	tmp = tp->rtl8125_rx_config | rx_mode |
...
void rtl8125_set_rx_q_num(struct rtl8125_private *tp,
...
void rtl8125_set_tx_q_num(struct rtl8125_private *tp,
...
void rtl8125_hw_config(struct net_device *dev)
{
	...
	if (tp->mcfg == CFG_METHOD_2 || tp->mcfg == CFG_METHOD_3 ||
		...
		mac_ocp_data &= ~(BIT_7 | BIT_6 | BIT_5 | BIT_4 | BIT_3 |
				  BIT_2 | BIT_1 | BIT_0);
	...
	if (tp->RequireRduNonStopPatch && tp->ShortPacketEmptyBuffer) {
		RTL_W32(tp, RSS_INDIRECTION_TBL_8125_V2,
			((u64)tp->ShortPacketEmptyBufferPhy &
			 DMA_BIT_MASK(32)));
		RTL_W32(tp, RSS_INDIRECTION_TBL_8125_V2 + 4,
			((u64)tp->ShortPacketEmptyBufferPhy >> 32));
	}

	if (tp->use_timer_interrrupt && (tp->HwCurrIsrVer == 2) &&
	    (tp->HwSuppIntMitiVer == 4) && (tp->features & RTL_FEATURE_MSIX)) {
		int i;
		for (i = 0; i < tp->irq_nvecs; i++)
	...
	if (!tp->pci_cfg_is_read) {
		...
		if (tp->HwPcieSNOffset > 0) {
			tp->pci_cfg_space.pci_sn_l =
				rtl8125_csi_read(tp, tp->HwPcieSNOffset);
			tp->pci_cfg_space.pci_sn_h =
				rtl8125_csi_read(tp, tp->HwPcieSNOffset + 4);
		}
...
static void rtl8125_free_rx_skb(struct rtl8125_private *tp,
				struct rtl8125_rx_ring *ring,
...
static inline void rtl8125_mark_to_asic_v3(struct RxDescV3 *descv3,
					   u32 rx_buf_sz)
{
	u32 eor = le32_to_cpu(descv3->RxDescNormalDDWord4.opts1) & RingEnd;

	WRITE_ONCE(descv3->RxDescNormalDDWord4.opts1,
		   cpu_to_le32(DescOwn | eor | rx_buf_sz));
}

void rtl8125_mark_to_asic(struct rtl8125_private *tp, struct RxDesc *desc,
			  u32 rx_buf_sz)
{
	if (tp->InitRxDescType == RX_DESC_RING_TYPE_3)
...
static inline void rtl8125_map_to_asic(struct rtl8125_private *tp,
				       struct rtl8125_rx_ring *ring,
				       struct RxDesc *desc, dma_addr_t mapping,
				       u32 rx_buf_sz, const u32 cur_rx)
{
...
static int rtl8125_alloc_rx_skb(struct rtl8125_private *tp,
				struct rtl8125_rx_ring *ring,
				struct sk_buff **sk_buff, struct RxDesc *desc,
				int rx_buf_sz, const u32 cur_rx, u8 in_intr)
{
	struct sk_buff *skb;
	dma_addr_t mapping;
	int ret = 0;
	...
	if (in_intr)
		skb = RTL_ALLOC_SKB_INTR(&tp->r8125napi[ring->index].napi,
...
static void _rtl8125_rx_clear(struct rtl8125_private *tp,
			      struct rtl8125_rx_ring *ring)
{
	int i;
...
static u32 rtl8125_rx_fill(struct rtl8125_private *tp,
			   struct rtl8125_rx_ring *ring, struct net_device *dev,
			   u32 start, u32 end, u8 in_intr)
{
	...
	for (cur = start; end - cur > 0; cur++) {
		...
		ret = rtl8125_alloc_rx_skb(
			tp, ring, ring->Rx_skbuff + i,
			rtl8125_get_rxdesc(tp, ring->RxDescArray, i),
			tp->rx_buf_sz, i, in_intr);
		if (ret < 0)
			break;
	}
	return cur - start;
...
static inline void rtl8125_mark_as_last_descriptor_8125(struct RxDescV3 *descv3)
{
	descv3->RxDescNormalDDWord4.opts1 |= cpu_to_le32(RingEnd);
}

static inline void rtl8125_mark_as_last_descriptor(struct rtl8125_private *tp,
						   struct RxDesc *desc)
{
	if (tp->InitRxDescType == RX_DESC_RING_TYPE_3)
...
static void rtl8125_desc_addr_fill(struct rtl8125_private *tp)
{
	int i;
...
static void rtl8125_tx_desc_init(struct rtl8125_private *tp)
{
	...
	for (i = 0; i < tp->num_tx_rings; i++) {
		struct rtl8125_tx_ring *ring = &tp->tx_ring[i];
		memset(ring->TxDescArray, 0x0,
		       (ring->num_tx_desc * sizeof(struct TxDesc)));

		ring->TxDescArray[ring->num_tx_desc - 1].opts1 =
			cpu_to_le32(RingEnd);
	}
}
...
int rtl8125_init_ring(struct net_device *dev)
{
	...
	for (i = 0; i < tp->num_rx_rings; i++) {
		...
		if (rtl8125_rx_fill(tp, ring, dev, 0, ring->num_rx_desc, 0) !=
		    ring->num_rx_desc)
			goto err_out;

		rtl8125_mark_as_last_descriptor(
			tp, rtl8125_get_rxdesc(tp, ring->RxDescArray,
					       ring->num_rx_desc - 1));
	}
...
static void rtl8125_unmap_tx_skb(struct pci_dev *pdev, struct ring_info *tx_skb,
				 struct TxDesc *desc)
{
...
static void rtl8125_tx_clear_range(struct rtl8125_private *tp,
				   struct rtl8125_tx_ring *ring, u32 start,
				   unsigned int n)
{
	unsigned int i;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 22)
...
void rtl8125_tx_clear(struct rtl8125_private *tp)
{
	...
	for (i = 0; i < tp->num_tx_rings; i++) {
		struct rtl8125_tx_ring *ring = &tp->tx_ring[i];
		rtl8125_tx_clear_range(tp, ring, ring->dirty_tx,
				       ring->num_tx_desc);
...
static void rtl8125_wait_for_irq_complete(struct rtl8125_private *tp)
{
	if (tp->features & RTL_FEATURE_MSIX) {
		int i;
		for (i = 0; i < tp->irq_nvecs; i++)
...
static void _rtl8125_wait_for_quiescence(struct net_device *dev)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static void rtl8125_reset_task(struct work_struct *work)
{
	...
	if (!netif_running(dev) || test_bit(R8125_FLAG_DOWN, tp->task_flags) ||
	...
	if (rtl8125_rx_nostuck(tp)) {
		...
		if (tp->resume_not_chg_speed) {
			...
			rtl8125_set_speed(dev, tp->autoneg, tp->speed,
...
static void rtl8125_esd_task(struct work_struct *work)
{
	...
	if (!netif_running(dev) || test_bit(R8125_FLAG_DOWN, tp->task_flags) ||
	    !test_and_clear_bit(R8125_FLAG_TASK_ESD_CHECK_PENDING,
				tp->task_flags))
		goto out_unlock;
...
static void rtl8125_linkchg_task(struct work_struct *work)
{
	...
	if (!netif_running(dev) || test_bit(R8125_FLAG_DOWN, tp->task_flags) ||
	    !test_and_clear_bit(R8125_FLAG_TASK_LINKCHG_CHECK_PENDING,
				tp->task_flags))
		goto out_unlock;
...
static u32 rtl8125_get_txd_opts1(struct rtl8125_tx_ring *ring, u32 opts1,
				 u32 len, unsigned int entry)
{
...
static int rtl8125_xmit_frags(struct rtl8125_private *tp,
			      struct rtl8125_tx_ring *ring, struct sk_buff *skb,
			      const u32 *opts)
{
...
#endif
		if (tp->RequireLSOPatch && (cur_frag == nr_frags - 1) &&
		    (opts[0] & (GiantSendv4 | GiantSendv6)) &&
		    PktLenCnt < ETH_FRAME_LEN && len > 1) {
			len -= 1;
			mapping = dma_map_single(tp_to_dev(tp), addr, len,
						 DMA_TO_DEVICE);

			if (unlikely(dma_mapping_error(tp_to_dev(tp),
						       mapping))) {
				if (unlikely(net_ratelimit()))
					netif_err(
						tp, drv, tp->dev,
			...
			status = rtl8125_get_txd_opts1(ring, opts[0], len,
						       entry);
...
static inline u8 rtl8125_get_l4_protocol(struct sk_buff *skb)
{
...
static u32 rtl8125_get_patch_pad_len(struct sk_buff *skb)
{
	u32 pad_len = 0;
	int trans_data_len;
	u32 hdr_len;
	...
	trans_data_len = pkt_len - (skb->transport_header - skb_headroom(skb));
...
static bool rtl8125_tso_csum(struct sk_buff *skb, struct net_device *dev,
			     u32 *opts)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
static bool rtl8125_tx_slots_avail(struct rtl8125_private *tp,
				   struct rtl8125_tx_ring *ring)
{
	unsigned int slots_avail = READ_ONCE(ring->dirty_tx) +
				   ring->num_tx_desc - READ_ONCE(ring->cur_tx);
				      ...
				      struct net_device *dev)
{
	...
	if (EnableTxNoClose)
		RTL_W16(tp, ring->sw_tail_ptr_reg,
			ring->cur_tx % MAX_TX_NO_CLOSE_DESC_PTR_V2);
	else
		RTL_W16(tp, TPPOLL_8125,
			BIT(ring->index)); /* set polling bit */
...
static inline u32 rtl8125_fast_mod(const u32 input, const u32 ceil)
{
...
static int rtl8125_tx_interrupt(struct rtl8125_tx_ring *ring, int budget)
{
	...
	if (EnableTxNoClose) {
		...
		smp_rmb();
		tx_desc_closed = rtl8125_fast_mod(
			NextHwDesCloPtr - ring->BeginHwDesCloPtr,
			MAX_TX_NO_CLOSE_DESC_PTR_V2);
		if (tx_left > tx_desc_closed)
			tx_left = tx_desc_closed;
...
static inline int rtl8125_fragmented_frame(struct rtl8125_private *tp,
					   u32 status)
{
	if (tp->InitRxDescType == RX_DESC_RING_TYPE_3)
		return (status & (FirstFrag_V3 | LastFrag_V3)) !=
		       (FirstFrag_V3 | LastFrag_V3);
	else
		return (status & (FirstFrag | LastFrag)) !=
		       (FirstFrag | LastFrag);
}

static inline int rtl8125_rx_desc_type(u32 status)
{
	return ((status >> 26) & 0x0F);
}

static inline void rtl8125_rx_v3_csum(struct rtl8125_private *tp,
				      struct sk_buff *skb,
...
static inline void rtl8125_rx_csum(struct rtl8125_private *tp,
...
static inline int rtl8125_try_rx_copy(struct rtl8125_private *tp,
				      struct rtl8125_rx_ring *ring,
...
static inline void rtl8125_rx_skb(struct rtl8125_private *tp,
				  struct sk_buff *skb, u32 ring_index)
{
#ifdef CONFIG_R8125_NAPI
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 29)
...
static int rtl8125_check_rx_desc_error(struct net_device *dev,
				       struct rtl8125_private *tp, u32 status)
{
	int ret = 0;
...
static int rtl8125_rx_interrupt(struct net_device *dev,
				struct rtl8125_private *tp,
				struct rtl8125_rx_ring *ring,
				napi_budget budget)
{
	...
	for (; rx_left > 0; rx_left--) {
		...
		if (unlikely(rtl8125_check_rx_desc_error(dev, tp, status) <
			     0)) {
		...
		process_pkt:
			...
			if (tp->EnablePtp) {
				...
				if (desc_type == RXDESC_TYPE_NEXT &&
					...
					entry_next = (cur_rx + 1) %
						     ring->num_rx_desc;
					desc_next = (struct RxDescV3 *)
						rtl8125_get_rxdesc(
							tp, ring->RxDescArray,
							entry_next);
					rmb();
					status_next = le32_to_cpu(
						desc_next->RxDescNormalDDWord4
							.opts1);
					...
					cur_rx++;
					rx_left--;
					desc_type = rtl8125_rx_desc_type(
						status_next);
					if (desc_type == RXDESC_TYPE_PTP)
						rtl8125_rx_ptp_pktstamp(
							tp, skb, desc_next);
					else
						WARN_ON(1);
			...
			dma_sync_single_for_cpu(tp_to_dev(tp), rx_buf_phy_addr,
						tp->rx_buf_sz, DMA_FROM_DEVICE);

			if (rtl8125_try_rx_copy(tp, ring, &skb, pkt_size, desc,
						tp->rx_buf_sz)) {
	...
	/*
         ...
         */
	if ((ring->dirty_rx + ring->num_rx_desc == ring->cur_rx) &&
	    netif_msg_intr(tp))
...
static bool rtl8125_linkchg_interrupt(struct rtl8125_private *tp, u32 status)
{
	if (tp->HwCurrIsrVer == 2)
...
#endif
{
	...
	do {
		...
		if (status & tp->intr_mask || tp->keep_intr_cnt-- > 0) {
			...
			if (likely(RTL_NETIF_RX_SCHEDULE_PREP(
				    dev, &tp->r8125napi[0].napi)))
				__RTL_NETIF_RX_SCHEDULE(dev,
							&tp->r8125napi[0].napi);
...
#endif
{
	...
	do {
		...
		if (likely(RTL_NETIF_RX_SCHEDULE_PREP(dev, &r8125napi->napi)))
			...
			printk(KERN_INFO
			       "%s: interrupt message id %d in poll_msix\n",
			       dev->name, message_id);
#else
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 24)
			rtl8125_rx_interrupt(dev, tp, &tp->rx_ring[message_id],
					     &budget);
#else
			rtl8125_rx_interrupt(dev, tp, &tp->rx_ring[message_id],
					     budget);
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 11)
static int rtl8125_suspend(struct pci_dev *pdev, u32 state)
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 29)
static int rtl8125_suspend(struct device *device)
#else
static int rtl8125_suspend(struct pci_dev *pdev, pm_message_t state)
#endif
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 29)
...
static int rtl8125_hw_d3_not_power_off(struct net_device *dev)
{
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 29)
static int rtl8125_resume(struct pci_dev *pdev)
#else
static int rtl8125_resume(struct device *device)
#endif
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 29)
	struct pci_dev *pdev = to_pci_dev(device);
