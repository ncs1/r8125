...
#define queue_delayed_work(long_wq, work, delay)                               \
...
#define RTL_NET_DEVICE_OPS(ops)                                                \
	dev->open = rtl8125_open;                                              \
...
#define RTL8125_VERSION                                                        \
	"9.010.01" NAPI_SUFFIX DASH_SUFFIX REALWOW_SUFFIX PTP_SUFFIX RSS_SUFFIX
...
#define napi dev
#define RTL_NAPI_CONFIG(ndev, priv, function, weig)                            \
	ndev->poll = function;                                                 \
	ndev->weight = weig;
...
struct rtl8125_private {
...
#else
	void (*get_settings)(struct net_device *,
	...
	u8 HwPkgDet;
	void __iomem *
		mapped_cmac_ioaddr; /* mapped cmac memory map physical address */
...
#ifdef ENABLE_LIB_SUPPORT
static inline unsigned int rtl8125_num_lib_tx_rings(struct rtl8125_private *tp)
{
	int count, i;
...
static inline unsigned int rtl8125_num_lib_rx_rings(struct rtl8125_private *tp)
{
	int count, i;
...
static inline unsigned int rtl8125_tot_tx_rings(struct rtl8125_private *tp)
{
	return tp->num_tx_rings + rtl8125_num_lib_tx_rings(tp);
}

static inline unsigned int rtl8125_tot_rx_rings(struct rtl8125_private *tp)
{
...
void rtl8125_mdio_prot_write(struct rtl8125_private *tp, u32 RegAddr,
			     u32 value);
...
static inline void rtl8125_make_unusable_by_asic(struct rtl8125_private *tp,
						 struct RxDesc *desc)
{
...
static inline struct RxDesc *rtl8125_get_rxdesc(struct rtl8125_private *tp,
						struct RxDesc *RxDescBase,
						u32 const cur_rx)
{
	return (struct RxDesc *)((u8 *)RxDescBase +
				 (cur_rx * tp->RxDescLength));
}

static inline void rtl8125_disable_hw_interrupt_v2(struct rtl8125_private *tp,
						   u32 message_id)
{
	RTL_W32(tp, IMR_V2_CLEAR_REG_8125, BIT(message_id));
}

static inline void rtl8125_enable_hw_interrupt_v2(struct rtl8125_private *tp,
						  u32 message_id)
{
