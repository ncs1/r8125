...
#define RTL8125_UDP_RSS_FLAGS                                                  \
	(RTL_8125_RSS_FLAG_HASH_UDP_IPV4 | RTL_8125_RSS_FLAG_HASH_UDP_IPV6)
static int _rtl8125_set_rss_hash_opt(struct rtl8125_private *tp)
{
	...
	rss_ctrl |= RSS_CTRL_TCP_IPV4_SUPP | RSS_CTRL_IPV4_SUPP |
		    RSS_CTRL_IPV6_SUPP | RSS_CTRL_TCP_IPV6_SUPP;
				    ...
				    struct ethtool_rxnfc *nfc)
{
	...
	if (rss_flags != tp->rss_flags) {
		...
		rss_ctrl &= ~(RSS_CTRL_UDP_IPV4_SUPP | RSS_CTRL_UDP_IPV6_SUPP);
...
int rtl8125_get_rxfh(struct net_device *dev, u32 *indir, u8 *key, u8 *hfunc)
{
	struct rtl8125_private *tp = netdev_priv(dev);
...
int rtl8125_set_rxfh(struct net_device *dev, const u32 *indir, const u8 *key,
		     const u8 hfunc)
{
	struct rtl8125_private *tp = netdev_priv(dev);
	int i;
