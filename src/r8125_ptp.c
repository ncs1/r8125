...
static int _rtl8125_phc_gettime(struct rtl8125_private *tp,
				struct timespec64 *ts64)
{
	//get local time
	RTL_W16(tp, PTP_TIME_CORRECT_CMD_8125,
		(PTP_CMD_LATCHED_LOCAL_TIME | PTP_EXEC_CMD));

	/* nanoseconds */
	//0x6808[29:0]
	ts64->tv_nsec =
		(RTL_R32(tp, PTP_SOFT_CONFIG_Time_NS_8125) & 0x3fffffff) +
		tp->ptp_adjust;
...
static int _rtl8125_phc_settime(struct rtl8125_private *tp,
				const struct timespec64 *ts64)
{
	...
	//set local time
	RTL_W16(tp, PTP_TIME_CORRECT_CMD_8125,
		(PTP_CMD_SET_LOCAL_TIME | PTP_EXEC_CMD));
...
static int rtl8125_phc_adjtime(struct ptp_clock_info *ptp, s64 delta)
{
	struct rtl8125_private *tp =
		container_of(ptp, struct rtl8125_private, ptp_clock_info);
	unsigned long flags;
	//int ret = 0;
...
static int _rtl8125_phc_adjfreq(struct ptp_clock_info *ptp, s32 ppb)
{
	...
	//adjust local time
	RTL_W16(tp, PTP_TIME_CORRECT_CMD_8125,
		(PTP_CMD_DRIFT_LOCAL_TIME | PTP_EXEC_CMD));
	//RTL_W16(tp, PTP_TIME_CORRECT_CMD_8125, (PTP_CMD_SET_LOCAL_TIME | PTP_EXEC_CMD));
...
static int rtl8125_phc_gettime(struct ptp_clock_info *ptp,
			       struct timespec64 *ts64)
{
	struct rtl8125_private *tp =
		container_of(ptp, struct rtl8125_private, ptp_clock_info);
...
static int rtl8125_get_tx_ptp_pkt_tstamp(struct rtl8125_private *tp,
					 struct timespec64 *ts64)
{
...
{
	struct rtl8125_private *tp =
		container_of(work, struct rtl8125_private, ptp_tx_work);
	unsigned long flags;
	...
	if (time_is_before_jiffies(tp->ptp_tx_start + RTL8125_PTP_TX_TIMEOUT)) {
...
static int rtl8125_hwtstamp_enable(struct rtl8125_private *tp, bool enable)
{
	...
	if (enable) {
		...
		rtl8125_mac_ocp_write(tp, 0xDC00,
				      rtl8125_mac_ocp_read(tp, 0xDC00) | BIT_6);
		//enable ptp
...
static long rtl8125_ptp_create_clock(struct rtl8125_private *tp)
{
	...
	tp->ptp_clock =
		ptp_clock_register(&tp->ptp_clock_info, &tp->pci_dev->dev);
			     ...
			     struct RxDescV3 *descv3)
{
	...
	tv_sec = le32_to_cpu(descv3->RxDescTimeStamp.TimeStampHigh) +
		 ((u64)le32_to_cpu(descv3->RxDescPTPDDWord4.TimeStampHHigh)
		  << 32);
